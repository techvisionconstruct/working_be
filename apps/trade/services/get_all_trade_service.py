...
def get_all_trade_service(
    user: User, search_query: Optional[str] = None, page: int = 1, page_size: int = 10
) -> Tuple[List[Trade], Dict]:
    # Base query: only trades created by the current user
    query = Q(created_by=user) & Q(origin="original")
    ...
    # Fetch filtered and sorted trades
    all_trades = Trade.objects.filter(query).order_by("-created_at")
    ...
    return current_page_trades, pagination_info
