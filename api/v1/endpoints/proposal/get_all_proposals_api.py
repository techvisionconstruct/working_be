...
from apps.proposal.dto import ProposalListResponse
from apps.proposal.services.get_all_proposals_service import get_all_proposals_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router(tags=["Proposals"])


@router.get("/", response=ProposalListResponse, auth=AuthBearer())
def get_all_proposals_api(
    request,
    search: Optional[str] = Query(None, description="Search term"),
    page: int = Query(1, description="Page number"),
    page_size: int = Query(10, description="Items per page"),
):
    # Get proposals and pagination info from service
    proposals, pagination_info = get_all_proposals_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved proposals. Found {pagination_info['total_count']} proposals",
        "data": proposals,
        "links": links,
