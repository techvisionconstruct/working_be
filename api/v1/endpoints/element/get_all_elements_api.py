...
from apps.element.dto import ElementListResponse
from apps.element.services.get_all_elements_service import get_all_elements_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router(tags=["Elements"])


@router.get("/", response=ElementListResponse, auth=AuthBearer())
def get_all_elements_api(
    request,
    search: Optional[str] = Query(None, description="Search term"),
    page: int = Query(1, description="Page number"),
    page_size: int = Query(10, description="Items per page"),
):
    # Get elements and pagination info from service
    elements, pagination_info = get_all_elements_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved elements. Found {pagination_info['total_count']} elements",
        "data": elements,
        "links": links,
