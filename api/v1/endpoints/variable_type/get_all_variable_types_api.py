...
from apps.variable_type.dto import VariableTypeListResponse
from apps.variable_type.services import get_all_variable_types_service
...
@router.get("/", response=VariableTypeListResponse, auth=AuthBearer())
def get_all_variable_types_api(
    request,
    search: Optional[str] = Query(None, description="Search term"),
    page: int = Query(1, description="Page number"),
    page_size: int = Query(10, description="Items per page"),
):
    # Get templates and pagination info from service
    variable_types, pagination_info = get_all_variable_types_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved variable types. Found {pagination_info['total_count']} variable types",
        "data": variable_types,
        "links": links,
