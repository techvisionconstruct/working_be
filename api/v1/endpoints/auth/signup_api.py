...
from apps.auth.dto import SignUpRequest, SignUpResponse
from apps.auth.services import signup_service
...
def signup_endpoint(request: HttpRequest, payload: SignUpRequest):
    user_data, error = signup_service(
        email=payload.email,
    ...
    return Response(response_data, status=201)  # Created
