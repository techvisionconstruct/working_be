...
from apps.trade.dto import TradeListResponse
from apps.trade.services.get_all_trade_service import get_all_trade_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router(tags=["Trades"])


@router.get("/", response=TradeListResponse, auth=AuthBearer())
def get_all_trades_api(
    ...
    # Get trades and pagination info from service
    trades, pagination_info = get_all_trade_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved trades. Found {pagination_info['total_count']} trades",
        "data": trades,
        "links": links,
