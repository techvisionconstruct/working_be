...
from apps.user_profile.dto import ProfileListResponse
from apps.user_profile.services import get_all_profiles_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router()


@router.get("", response=ProfileListResponse, auth=AuthBearer())
def get_all_profiles_api(
    request,
    search: Optional[str] = Query(None, description="Search term for user profiles"),
    page: int = Query(1, description="Page number"),
    page_size: int = Query(10, description="Items per page"),
):
    # Get profiles and pagination info from service
    profiles, pagination_info = get_all_profiles_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved profiles. Found {pagination_info['total_count']} profiles",
        "data": profiles,
        "links": links,
