...
from apps.template.dto import TemplateListResponse
from apps.template.services import get_all_templates_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router(tags=["Templates"])


@router.get("/", response=TemplateListResponse, auth=AuthBearer())
def get_all_templates_api(
    ...
    templates, pagination_info = get_all_templates_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved templates. Found {pagination_info['total_count']} templates",
        "data": templates,
        "links": links,
