...
from apps.industry.dto import IndustryListResponse
from apps.industry.services import get_all_industries_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router()


@router.get("/", response=IndustryListResponse, auth=AuthBearer())
def get_all_industries_api(
    request,
    search: Optional[str] = Query(
        None, description="Search term for industry name or description"
    ),
    page: int = Query(1, description="Page number"),
    page_size: int = Query(10, description="Items per page"),
):
    # Get industries and pagination info from service
    industries, pagination_info = get_all_industries_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved industries. Found {pagination_info['total_count']} industries",
        "data": industries,
        "links": links,
