...
from apps.product.dto import ProductListResponse
from apps.product.services import get_all_products_service
from apps.auth.services.authenticate_service import AuthBearer

router = Router(tags=["Products"])


@router.get("/", response=ProductListResponse, auth=AuthBearer())
def get_all_products_api(
    ...
    # Get products and pagination info from service
    products, pagination_info = get_all_products_service(
    ...
    return {
        "success": True,
        "message": f"Successfully retrieved products. Found {pagination_info['total_count']} products",
        "data": products,
        "links": links,
